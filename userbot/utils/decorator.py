    ...
    **args,
    ...
    if pattern is not None:
        ...
        if (
            ...
            galon_reg = sudo_reg = re.compile(pattern)
        else:
            galon_ = "\\" + CMD_HANDLER
            sudo_ = "\\" + SUDO_HANDLER
            galon_reg = re.compile(galon_ + pattern)
        ...
        if not disable_edited:
            bot.add_event_handler(
                func, events.MessageEdited(
                    **args, outgoing=True, pattern=galon_reg))
        bot.add_event_handler(
            func, events.NewMessage(**args, outgoing=True, pattern=galon_reg)
        )
        if allow_sudo:
            if not disable_edited:
                bot.add_event_handler(
